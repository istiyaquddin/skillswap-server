...
async function run() {
  try {
    ...
    const database = client.db(process.env.DB_NAME || "freelance_db");
    ...
    app.patch("/api/admin/users/:id/status", verifyToken, async (req, res) => {
      try {
        ...
        res
          .status(200)
          .json({ success: true, message: `User status updated to ${status}` });
      } catch (error) {
    ...
    app.get("/api/admin/tasks", verifyToken, async (req, res) => {
      try {
        ...
        const formattedTasks = tasks.map((task) => {
          ...
          let cleanCategory = task.category
    ...
    app.post("/api/payment", async (req, res) => {
      try {
        const { sessionId, userId, userEmail, priceId, taskId, proposalId } =
          req.body;
        ...
        if (isExist) {
          return res
            .status(400)
            .json({ success: false, msg: "Already Exist!" });
        }

        if (!ObjectId.isValid(taskId)) {
          return res
            .status(400)
            .json({ success: false, msg: "Invalid Task ID format" });
        }
        ...
        await paymentCollection.insertOne({
          ...
          proposalId:
            proposalId && ObjectId.isValid(proposalId)
        ...
        if (proposalId && ObjectId.isValid(proposalId)) {
          ...
          options = {
            arrayFilters: [
              ...
              {
                "other.proposalId": { $ne: new ObjectId(proposalId) },
    ...
    app.get("/api/payment-history", verifyToken, async (req, res) => {
      try {
        ...
        if (!email) {
          return res
            .status(400)
            .send({ success: false, message: "Client email is required" });
        }
                ...
                history: [
                  {
                    $project: {
                      ...
                      createdAt: 1,
                      taskTitle: {
                        $ifNull: ["$taskDetails.title", "Unknown Task"],
                      },
    ...
    app.get("/api/freelancer-earnings", verifyToken, async (req, res) => {
      try {
        ...
        allTasks.forEach((task) => {
          ...
          if (hasFreelancerProposal) {
            ...
            if (matchedPayment) {
              ...
              const paymentDate = matchedPayment.createdAt
                ? new Date(matchedPayment.createdAt)
                : new Date();
        ...
        const avgEarned =
          paymentCount > 0 ? Math.round(totalEarned / paymentCount) : 0;

        console.log(
          `Matched ${paymentCount} successfully paid tasks for freelancer.`,
        );
    ...
    app.get("/api/freelancers/:id", verifyToken, async (req, res) => {
      try {
        ...
        if (!ObjectId.isValid(id)) {
          return res
            .status(400)
            .send({ error: true, message: "Invalid Freelancer ID format" });
        }
        ...
        if (!freelancer) {
          return res
            .status(404)
            .send({ error: true, message: "Freelancer not found" });
        }
        res.send(freelancer);
      } catch (error) {
    ...
    app.get("/api/clients/:id", verifyToken, async (req, res) => {
      try {
        ...
        if (!ObjectId.isValid(id)) {
          return res
            .status(400)
            .send({ error: true, message: "Invalid Client ID format" });
        }
        ...
        if (!clientData) {
          return res
            .status(404)
            .send({ error: true, message: "Client not found" });
        }
        res.send(clientData);
      } catch (error) {
    ...
    app.get("/api/admins/:id", verifyToken, async (req, res) => {
      try {
        ...
        if (!ObjectId.isValid(id)) {
          return res
            .status(400)
            .send({ error: true, message: "Invalid Admin ID format" });
        }
    ...
    app.get("/api/tasks/:id", async (req, res) => {
      try {
        ...
        if (!ObjectId.isValid(id)) {
          return res
            .status(400)
            .send({ error: true, message: "Invalid Task ID format" });
        }
        const taskArray = await tasksCollection
          .aggregate([
    ...
    app.get("/api/tasks", async (req, res) => {
      try {
        const { status, search, category, minBudget, maxBudget, page, limit } =
          req.query;
        const currentPage = parseInt(page) || 1;
        ...
        const aggregationResult = await tasksCollection
          .aggregate(pipeline)
          .toArray();
    ...
    app.get("/api/my-proposals", verifyToken, async (req, res) => {
      try {
        ...
        const proposals = await tasksCollection
          .aggregate([
    ...
    app.put(
      "/api/proposals/:taskId/:proposalId",
      verifyToken,
      async (req, res) => {
        try {
    ...
    app.get(
      "/api/proposals/details/:proposalId",
      verifyToken,
      async (req, res) => {
        try {
          ...
          const proposalData = await tasksCollection
            .aggregate([
    ...
    app.patch("/api/tasks/complete/:id", async (req, res) => {
      try {
        ...
        if (!ObjectId.isValid(id)) {
          return res
            .status(400)
            .send({ success: false, message: "Invalid Task ID format" });
        }
        if (!deliverableUrl) {
          return res
            .status(400)
            .send({ success: false, message: "Deliverable URL is required" });
        }
